...
const ManageResources = () => {
  ...
  const all = useMemo(() => resourceRepo.list(), [tick]);
  ...
  const save = () => {
    ...
    if (editing) {
      resourceRepo.update(editing.id, { ...draft, external: draft.link.startsWith('http') });
              ...
              {all.map((r) => (
                <li key={r.id} className="py-2.5 flex items-center justify-between gap-3">
                  ...
                  <div className="flex gap-1 shrink-0">
                    ...
                    {r.status === 'Active' ? (
                      <Button size="sm" variant="ghost" onClick={() => { resourceRepo.archive(r.id); setTick((t) => t + 1); }}>
                        <Archive className="h-3.5 w-3.5" />
                      </Button>
                    ) : (
                      <Button size="sm" variant="ghost" onClick={() => { resourceRepo.unarchive(r.id); setTick((t) => t + 1); }}>
        ...
        <Card>
          ...
          <CardContent className="space-y-3">
            ...
            <div className="flex gap-2">
              <Button onClick={save}><Plus className="h-3.5 w-3.5 mr-1" />{editing ? 'Save' : 'Add'}</Button>
